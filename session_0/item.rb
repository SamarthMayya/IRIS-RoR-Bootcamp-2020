class Item
  ...
  def self.stock_price_by_category(items)
    sp = Hash.new 
    sp = {
      1 => items.sum{|item| item.category_id == 1 && item.quantity && item.current_price ? item.current_price*item.quantity : 0  },
      2 => items.sum{|item| item.category_id == 2 && item.quantity && item.current_price ? item.current_price*item.quantity : 0  },
      3 => items.sum{|item| item.category_id == 3 && item.quantity && item.current_price ? item.current_price*item.quantity : 0  },
      4 => items.sum{|item| item.category_id == 4 && item.quantity && item.current_price ? item.current_price*item.quantity : 0  }
    }
  end
end
